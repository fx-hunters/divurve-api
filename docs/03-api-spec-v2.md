...
| `quota_user` | 이 사용자가 24시간 상한을 다 썼다 (데모는 더 낮다) | `null` (호출 자체를 안 했다) | `[]` |
| `quota_ip` | 이 IP 에서 온 호출이 상한을 넘었다 | `null` (호출 자체를 안 했다) | `[]` |
| `quota_global` | 서비스 전체가 24시간 상한을 넘었다 — 킬스위치 | `null` (호출 자체를 안 했다) | `[]` |
...
#### 총량 상한 — 넘으면 **429 가 아니라 200 + `fallback: true`** (이슈 #140)

호출 횟수에 상한이 없었다. 요청당 타임아웃(5s)·총예산(8s)은 **1건의 지연**을, 입력 상한·캐시(#139)는
**1건당 비용과 중복**을 막는 장치이지 총량을 막지 못한다. 세 층을 얹었다.

| 층 | 무엇을 세는가 | 기본 상한 | 넘으면 |
|---|---|---|---|
| 사용자 | 이 사용자의 유료 호출 | 100건 / 24h (**데모 30건**) | `quota_user` |
| IP | 이 IP 에서 온 유료 호출 | 300건 / 24h | `quota_ip` |
| 전역 | 서비스 전체 (narrate + extract) | 500건 / 24h | `quota_global` |

- **429 를 새로 만들지 않는다.** 명세 §1.3 의 에러코드는 6종 닫힌 집합이고, FR-AI-06 은 AI 를
  부르지 못한 것이 서비스 실패가 아니라고 정한다. 기존 폴백 경로를 그대로 쓰므로 **프론트에 새
  분기가 필요하지 않다** — 이미 다루는 `fallback: true` 에 `fallback_reason` 값 셋이 늘 뿐이다.
- **세는 것은 `success` 와 `fallback` 뿐이다.** `cache_hit` 은 비용이 0 이라 세지 않는다 — 그래서
  **캐시 히트는 쿼터를 소모하지도, 쿼터에 막히지도 않는다**(담아 둔 문장을 주는 것이 폴백 템플릿을
  주는 것보다 언제나 낫다). `quota_blocked` 도 세지 않는다.
- **창은 달력상의 하루가 아니라 지난 24시간**이다. 관리자 집계는 UTC 로 일자를 자르고 서비스 시계는
  KST 라, "오늘" 로 세면 두 숫자가 9시간 동안 어긋난다. 자정을 기다려 상한을 두 배로 쓰는 우회도 없다.
- 상한은 환경변수(`AI_QUOTA_*`)로 조정한다.
- **IP 층의 한계를 분명히 한다.** 근거가 `X-Forwarded-For` 이고 그 헤더는 클라이언트가 위조할 수
  있다 — IP 층은 실수로 도는 루프와 소박한 스크립트를 막는 **과속방지턱**이다. 위조로 우회할 수 없는
  상한은 **전역 킬스위치** 하나다.

##### `POST /auth/demo` 발급 상한 — **403**

데모 세션은 회원가입 없이 발급되므로, 상한이 없으면 데모 토큰을 반복 발급해 사용자 층을 우회할 수
있다(토큰마다 다른 사용자라 카운트가 매번 0 에서 시작한다). **IP당 1시간에 20건**(`DEMO_ISSUANCE_PER_IP_HOURLY`)
을 넘으면 `403 FORBIDDEN` 이다 — §1.3 이 에러코드를 6종으로 닫아 두었으므로 레이트리밋용 코드를
새로 만들지 않는다. IP 를 알 수 없는 요청에는 적용하지 않는다.
...
| 총량 상한(사용자·IP·전역)을 넘으면 429 가 아니라 200 + `fallback_reason=quota_*` 다 | 이슈 #140, FR-AI-06 |
| 캐시 히트는 쿼터를 소모하지도, 쿼터에 막히지도 않는다 | 이슈 #140 |
